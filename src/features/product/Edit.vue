...
const onSubmit = handleSubmit(async (values) => {
  try {
    await put(`/products/${route.params.id}`, values)
    ...
    <CardWrapper replace-shadow-with-border>
      <form @submit.prevent="onSubmit" class="grid gap-[var(--m)]">
        ...
        <div class="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-[var(--m)]">
          ...
          <FormField v-model="price"    id="price"    label="Harga (Rp)" type="number" :error="errors.price" />
          <FormField v-model="stock"    id="stock"    label="Stok" type="number"       :error="errors.stock" />
        ...
        <FormField v-model="description" id="description" label="Deskripsi" type="textarea" :rows="4" :error="errors.description" />
